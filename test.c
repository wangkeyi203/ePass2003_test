...
char *g_strUserPIN;
CK_BBOOL bTrue = TRUE;
CK_ULONG ulModulusBits = 1024;  //MODULUS_BIT_LENGTH;
...
void CPKCSDemo::Encrypt()
{
   ...
   // m_pbCipherBuffer = (CK_BYTE_PTR)new CK_BYTE[m_ulCipherLen];
    m_pbCipherBuffer = (CK_BYTE_PTR)malloc(sizeof(CK_BYTE)*m_ulCipherLen);
    if (! m_pbCipherBuffer)
    {
...
void CPKCSDemo::Decrypt()
{
    ...
    pbRestoredMsg = (CK_BYTE_PTR)malloc(sizeof(CK_BYTE)*(ulRestoredMsgLen+1));
    if (! pbRestoredMsg)
    {
